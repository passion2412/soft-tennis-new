...
icon_image = Image.open(icon_path) if os.path.exists(icon_path) else "🎾"
...
# --- 2. HTMLコード (f-stringを使わず結合) ---
# Pythonの変数埋め込みを最小限にするため、テンプレートを分離します
pwa_head = """
...
<body>
    ...
    <script>
        ...
        function setMatchType(n) { state.match_type = n; render(); }
        ...
        function finishGame() {
            ...
            state.history.push({ score: state.p1 + "-" + state.p2 + (state.is_final ? "(F)" : ""), side: side });
...
# --- 3. 結合して表示 ---
full_html = html_body.replace("{pwa_head_tags}", pwa_head)
